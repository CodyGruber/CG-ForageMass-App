...
# ── Configuration ─────────────────────────────────────────────────────────────
# Edit pasture names here
PASTURE_NAMES <- c("Pasture 1", "Pasture 2", "Pasture 3")
...
loading_modal <- function(msg = "Fetching weather data from Mesonet…") {
  modalDialog(
    title = NULL, footer = NULL, easyClose = FALSE, fade = TRUE,
    tags$div(
      ...
      tags$p(tags$strong(msg), style = "color: #1a6e3c; font-size: 15px; margin: 0;"),
      tags$p("This may take a few seconds…", style = "color: #aaa; font-size: 12px; margin-top: 6px;")
    )
  )
}
...
plot_description_box <- function(text) {
  tags$div(
    style = paste(
      "background-color: #eef7f0;", "border-left: 4px solid #2a86c8;",
      "border-radius: 4px;", "padding: 10px 14px;", "margin-bottom: 10px;",
      "color: #1a5c38;", "font-family: Georgia, serif;",
...
# ── Per-pasture UI module ─────────────────────────────────────────────────────
pastureUI <- function(id) {
  ns <- NS(id)
  sidebarLayout(
    sidebarPanel(
      tabsetPanel(
        ...
        # ── Forage tab ──────────────────────────────────────────────────────
        tabPanel("Forage Data",
                 br(),
                 h4("Manual Entry"),
                 textInput(ns("Date"), "Date (YYYY-MM-DD or MM/DD/YYYY)"),
                 numericInput(ns("AvgPlateMeterReading"), "Avg Plate Meter Reading", value = NA),
                 actionButton(ns("add_row"),     "Add Row",              class = "btn-primary"),
                 ...
                 tags$hr(),
                 verbatimTextOutput(ns("gdd_status"))
        ),
        ...
        tabPanel("Animal Data",
                 br(),
                 h4("Add Animal"),
                 textInput(ns("animal_id"),     "Animal ID"),
                 numericInput(ns("animal_weight"), "Weight (lbs)", value = NA, min = 0),
                 actionButton(ns("add_animal"),    "Add Animal",           class = "btn-primary"),
                 actionButton(ns("delete_animal"), "Delete Selected",      class = "btn-danger"),
                 tags$hr(),
                 h4("Upload Animal CSV"),
                 tags$p("CSV should have columns: AnimalID, Weight_lbs",
                        style = "color: #888; font-size: 12px; margin-top: -6px;"),
                 fileInput(ns("animal_file"), "Choose CSV File",
                           accept = c("text/csv", ".csv")),
                 tags$hr(),
                 tags$div(
                   style = "background-color: #eef7f0; border: 1px solid #9fd4b2; border-radius: 4px; padding: 10px;",
                   h4("Herd Summary", style = "margin-top: 0;"),
                   verbatimTextOutput(ns("animal_summary"))
                 )
        )
      )
    ),
...
# ── Per-pasture server module ─────────────────────────────────────────────────
pastureServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    # ── Forage data ───────────────────────────────────────────────────────
    forage_data <- reactiveVal(data.frame(
      Date = character(), AvgPlateMeterReading = numeric()
    ))
    
    observeEvent(input$add_row, {
      req(input$Date, input$AvgPlateMeterReading)
      df  <- forage_data()
      new <- data.frame(Date = input$Date, AvgPlateMeterReading = input$AvgPlateMeterReading)
      forage_data(rbind(df, new))
    })
    
    observeEvent(input$delete_rows, {
      req(input$contents_rows_selected)
      df <- forage_data()
      forage_data(df[-input$contents_rows_selected, ])
    })
    ...
    observeEvent(input$file1, {
      ...
      forage_data(do.call(rbind, df_list))
    })
    ...
    # ── Animal data ───────────────────────────────────────────────────────
    animal_data <- reactiveVal(data.frame(
      AnimalID = character(), Weight_lbs = numeric()
    ))
    
    observeEvent(input$add_animal, {
      req(input$animal_id, input$animal_weight)
      df  <- animal_data()
      new <- data.frame(AnimalID = input$animal_id, Weight_lbs = input$animal_weight)
      animal_data(rbind(df, new))
      updateTextInput(session, "animal_id", value = "")
      updateNumericInput(session, "animal_weight", value = NA)
    })
    
    observeEvent(input$delete_animal, {
      req(input$animal_table_rows_selected)
      df <- animal_data()
      animal_data(df[-input$animal_table_rows_selected, ])
    })
    
    observeEvent(input$animal_file, {
      req(input$animal_file)
      df <- read.csv(input$animal_file$datapath)
      # Normalize column names
      names(df) <- trimws(names(df))
      if (!all(c("AnimalID", "Weight_lbs") %in% names(df))) {
        showNotification("CSV must have columns: AnimalID, Weight_lbs", type = "error")
        return()
      }
      animal_data(df[, c("AnimalID", "Weight_lbs")])
    })
    
    output$animal_table <- renderDT({
      df <- animal_data()
      if (nrow(df) == 0) return(datatable(df))
      df$AU <- round(df$Weight_lbs / 1000, 3)
      datatable(df,
                rownames = FALSE,
                editable = TRUE,
                selection = "multiple",
                colnames = c("Animal ID", "Weight (lbs)", "Animal Units (AU)"),
                options = list(pageLength = 15, dom = "tip"))
    })
    
    output$animal_summary <- renderText({
      df <- animal_data()
      if (nrow(df) == 0) return("No animals entered yet.")
      df$AU <- df$Weight_lbs / 1000
      total_head <- nrow(df)
      total_au   <- sum(df$AU, na.rm = TRUE)
      avg_wt     <- mean(df$Weight_lbs, na.rm = TRUE)
      paste0(
        "Head count : ", total_head, "\n",
        "Total AU   : ", round(total_au, 2), "\n",
        "Avg weight : ", round(avg_wt, 0), " lbs\n",
        "\nNote: 1 AU = 1,000 lb animal"
      )
    })
    ...
    fetch_mesonet_pkg <- function(start_date, end_date) {
      tryCatch({
        raw   <- mesonet::mnet_retrieve(stid = "STIL",
    ...
    gdd_data <- reactive({
      ...
      showModal(loading_modal(paste0("Fetching weather data via ", src_label, "…")))
      ...
      if (nrow(weather) == 0) { gdd_status("⚠ No rows matched date range."); return(NULL) }
    ...
    output$forecast_table <- renderDT({
      ...
      datatable(display, rownames = FALSE, options = list(pageLength = 10, dom = "t"),
    ...
    output$plot3 <- renderPlot({
      ...
      lines(trend_dates[obs_idx],   trend_forage[obs_idx],   col = "#1a6e3c", lwd = 2, lty = 1)
      lines(trend_dates[fcast_idx], trend_forage[fcast_idx], col = "#145aab", lwd = 2, lty = 2)
...
# ── UI ────────────────────────────────────────────────────────────────────────
ui <- navbarPage(
  title = tags$span("Forage Mass Tracker & GDD Calibration",
                    style = "color: #1a6e3c; font-weight: bold;"),
  theme = NULL,
  id = "main_navbar",
  
  # Inject global styles + shinyjs
  header = tagList(
    useShinyjs(),
    tags$head(tags$style(HTML("
    body { background-color: #ffffff; color: #0d2b1a; font-family: Georgia, serif; }
    .navbar { background-color: #eef7f0 !important; border-bottom: 2px solid #9fd4b2; }
    .navbar-brand { color: #1a6e3c !important; }
    .nav-tabs > li > a { color: #1a5c38; font-weight: bold; }
    .nav-tabs > li.active > a { background-color: #eef7f0 !important; border-top: 3px solid #2a86c8 !important; }
    .well { background-color: #eef7f0 !important; border-color: #9fd4b2 !important; }
    h4 { color: #1a6e3c; font-weight: bold; }
    .btn-primary {
      background-color: #2a86c8 !important; border-color: #1a6aab !important; color: #ffffff !important;
    }
    .btn-primary:hover { background-color: #1a6aab !important; }
    .btn-danger {
      background-color: #1a6e3c !important; border-color: #145730 !important; color: #ffffff !important;
    }
    .btn-danger:hover { background-color: #145730 !important; }
    .shiny-input-container label { color: #1a5c38; font-weight: bold; }
    pre { background-color: #eef7f0; border: 1px solid #9fd4b2; color: #1a5c38; }
    .modal-content { border: 2px solid #2a86c8; border-radius: 8px; }
    hr { border-color: #9fd4b2; }
    .dataTables_wrapper { color: #0d2b1a; }
    .tabbable > .nav > li > a { color: #1a5c38; }
    .tabbable > .nav > li.active > a { background-color: #2a86c8 !important; color: #fff !important; border-color: #1a6aab; }
    .rename-panel { max-width: 420px; margin: 30px auto; }
    .rename-panel .form-group { margin-bottom: 14px; }
  ")))
  ),
  
  # Dynamically generate one navbarPage tab per pasture
  !!!lapply(seq_along(PASTURE_NAMES), function(i) {
    tabPanel(
      title = PASTURE_NAMES[i],
      value = paste0("pasture_tab_", i),
      pastureUI(paste0("pasture", i))
    )
  }),
  
  # ── Rename Tabs panel ───────────────────────────────────────────────────
  tabPanel(
    title = tags$span("⚙ Rename Tabs", style = "color: #2a86c8;"),
    value = "rename_panel",
    tags$div(
      class = "rename-panel",
      h4("Rename Pasture Tabs"),
      tags$p("Type a new name and press Rename. Changes take effect immediately.",
             style = "color: #888; font-size: 13px; margin-top: -6px; margin-bottom: 20px;"),
      tagList(lapply(seq_along(PASTURE_NAMES), function(i) {
        tags$div(
          style = "display: flex; align-items: center; gap: 10px; margin-bottom: 12px;",
          tags$div(style = "flex: 1;",
                   textInput(paste0("rename_pasture_", i),
                             label    = paste0("Tab ", i, " name"),
                             value    = PASTURE_NAMES[i],
                             width    = "100%")
          ),
          tags$div(style = "padding-top: 24px;",
                   actionButton(paste0("do_rename_", i), "Rename", class = "btn-primary")
          )
        )
      }))
    )
...
server <- function(input, output, session) {
  for (i in seq_along(PASTURE_NAMES)) {
    pastureServer(paste0("pasture", i))
  }
}
